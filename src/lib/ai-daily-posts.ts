...
export function getAiDailyReports(): AiDailyMeta[] {
  const reports: AiDailyMeta[] = []
...
export function getAiDailyReport(slug: string): AiDailyPost | null {
...
export function getLatestAiDailyReport(): AiDailyPost | null {
  const reports = getAiDailyReports()
  if (reports.length === 0) return null
  return getAiDailyReport(reports[0].slug)
}
